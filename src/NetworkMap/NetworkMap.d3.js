...
export class NetworkMap {
    ...
    createTree() {
        ...
        if (isTree) {
            tree = d3.tree().size([2 * Math.PI, radius - 100])
        ...
        h.each(d => {
            if (d.data.name) {
                if (d.data.name.length > 30) {
                    d.data._name = `(${d.value})</br>` + d.data.name
                } else {
                    d.data._name = d.data.name + `</br>(${d.value})`
                }
    ...
    drawNodes({ dropShadowId, firstRender, transitionEase, duration, chart, tip, root, nodesWrapper, nodesData, source, rootCircleRadius, normalCircleRadius, svgWidth }) {
        const that = this;
        ...
        nodeUpdate.each(function (gd) {
            d3.select(this)
                .patternify({ tag: 'foreignObject', selector: 'text-fo', data: d => [gd] })
                ...
                .attr('x', d => {
                    ...
                    if (d.data.children || d.data._children) return -normalCircleRadius + 1;
                ...
                .html(
                    d => `
                            <div style="color:${d.data.children || d.data._children ? 'white' : 'black'};display: table; width:${Math.max((normalCircleRadius - 1) * 2, 0)}px; height: ${31 * 2 - 4}px; overflow: hidden;">
                               <div style="display: table-cell; vertical-align: middle;text-align:${d._textAlign}">
                                 <div style="opacity:1" class="node-name-wrapper">
        ...
        `
            ...
            .on('end', d => {
                nodeUpdate.selectAll('.node-circle').style('filter', `url(#drop-shadow-id)`)
            })
    ...
    drawLinks({ initialLinkColor, transitionEase, duration, chart, root, linksWrapper, linksData, source }) {
    ...
    drawSvgAndWrappers() {
        ...
        svg.patternify({ tag: 'rect', selector: 'boundary-rect' })
            .attr('width', svgWidth)
            .attr('height', svgHeight)
            .attr('fill', 'none')
        ...
        //     .textFill('black')
        //     .onNodeClick(d => {
        //         this.handleNodeClick({ event: null, d: d.node, state: this.getState() });
        //         this.handleNodeMouseEnter({ event: null, d: d.node, state: this.getState() })
        //     })
        //     .container(svg)
       ...
       // svg.call(tip);
        ...
        this.setState({ chart, svg, linksWrapper, nodesWrapper, tip, breadcrumb });
    }
    ...
    handleNodeMouseEnter({
        event,
        d: nodeData,
        state: { svgNodeGroups, svgLinks, breadcrumb, onNodeMouseEnter }
    }) {
        ...
        svgNodeGroups
            ...
            .raise()
            .attr('cursor', d =>
                d.data.children || d.data._children ? 'pointer' : (Math.round(Math.random()) ? 'no-drop' : 'copy')
            )
            .select('circle')
            .attr('fill', '#1493C8')
        ...
        const nodes = ancestors.map(d => Object.assign({}, template, { text: (d.data.name || d.data.title), node: d }));
        breadcrumb.show(nodes.reverse());
    }

    handleNodeMouseLeave({ event, d, state: { initialLinkColor, svgNodeGroups, svgLinks, breadcrumb, normalCircleRadius, rootCircleRadius, onNodeMouseLeave } }) {
        onNodeMouseLeave(d)
        svgLinks.classed('active', false).attr('stroke', initialLinkColor).attr('stroke-width', 1.5);
        svgNodeGroups
    ...
    limitMaximumVisibleNodes({ max, node, root }) {
        ...
        let currentNodeCount = descendants.length + (node.data.children || []).length;
    ...
    open(hierarchicalNode) {
        ...
        const node = state.root.descendants().filter(d => d.data == hierarchicalNode.data)[0];
        ...
        const nodeNew = state.root.descendants().filter(d => d.data == hierarchicalNode.data)[0];

        this.handleNodeMouseLeave({ event: null, d: nodeNew, state: this.getState() })
        this.handleNodeMouseEnter({ event: null, d: nodeNew, state: this.getState() })
